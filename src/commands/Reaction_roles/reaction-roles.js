...
async function handleDashboard(interaction, selectedPanelId) {
    ...
    if (!panels?.length) {
        throw createError(
            'No panels',
            ErrorTypes.CONFIGURATION,
            'No reaction role panels found. Use `/reaction-roles setup` first.',
        );
    }
            ...
            if (panels.length === 0) {
                await InteractionHelper.safeEditReply(rootInteraction, {
                    embeds: [
                        new EmbedBuilder()
                            .setTitle('Reaction Roles Dashboard')
                            .setDescription('No panels remain. Use `/reaction-roles setup` to create one.')
                            .setColor(getColor('info')),
                    ],
                    components: [],
                    flags: DASHBOARD_EPHEMERAL,
                ...
                await InteractionHelper.safeEditReply(rootInteraction, {
                    embeds: [
                        new EmbedBuilder()
                            .setTitle('Reaction Roles Dashboard')
                            .setDescription('Panel deleted. Run `/reaction-roles dashboard` to manage another panel.')
